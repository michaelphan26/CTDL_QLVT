...
int XoaVT(TREE_VATTU& tree, char *deleteKey)
{
	xoaKhungThongBao();
...
char* NameOfVatTu(TREE_VATTU ds_vt, char *maVT){
	if (stricmp(ds_vt->info.MAVT,maVT)!=0){
		NameOfVatTu(ds_vt->left,maVT);
		NameOfVatTu(ds_vt->right,maVT);
	}
	else return ds_vt->info.TENVT;
}
...
int ChuyenCayQuaMang(TREE_VATTU &ds_vt){
	
}

void GiaoDienDSVatTu(TREE_VATTU &ds_vt){
	...
	while (true){
		...
		if(i>28){
			if (y==14 && dem==0){
				...
				if (c==60){	//F2
					check=XoaVT(ds_vt,arrtemp[dem]->MAVT);
					xoaKhungDSNhanVien();
					if(check==1){	//Co xoa
						for(int j=dem;j<=i;j++){
