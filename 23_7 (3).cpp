...
void khoiTaoChiTietHD(DS_CTHD& cthd) {
	cthd.soluong=0;
}
...
int InsertLastCTHD(DS_CTHD& dscthd, CHITIET_HOADON cthd) { //tra ve vi tri them phan tu, tra ve -1 khi full ds
	if (dscthd.soluong >= MAXVT) {
		//cout << "TOI DA 20 VAT TU TRONG DS CTHD!" << endl;
		return -1;
	}
	dscthd.nodes[dscthd.soluong] = cthd;
	dscthd.soluong++;
	return dscthd.soluong - 1;
}

void khoiTaoHoaDon(NODES_HOADON& first) {
	first=NULL;
}
...
void nhapHoaDonCuoi(NODES_HOADON& first) {
	...
	while (true) {
		...
		if(checkDateTime(hd.date)==-1) break;
		xoaKhungThongBao();
	}
...
void InsertLastHoaDon(NODES_HOADON& first, HOADON& hoadon) { // nhap vao cuoi
	NODES_HOADON p, last; 
	if (first != NULL) {
		for ( last = first ; last->next !=NULL; last=last->next) ;
	}
...
int InsertLastNhanVien(DS_NV& dsnv, NHANVIEN add) {
	dsnv.nodes[dsnv.soluong] = new NHANVIEN;
	*dsnv.nodes[dsnv.soluong] = add;
	dsnv.soluong++;
	return 0;
}
...
void LineToNhanVien(string line, NHANVIEN& addNhanVien)
{
	...
	strcpy(addNhanVien.HO, temp.c_str());
	...
	strcpy(addNhanVien.TEN, temp.c_str());
	...
	strcpy(addNhanVien.PHAI, temp.c_str());
	...
	strcpy(addNhanVien.CMND, temp.c_str());
	i++;
	temp = "";
	
	khoiTaoHoaDon(addNhanVien.PTR_HD); //khoi tao DS Hoa Don
	...
	while (line[i] != '@')
	{
		...
		strcpy(addHoaDon.SOHD, temp.c_str());
		...
		addHoaDon.date.month = stoi(temp);
		...
		while (line[i] != '#')
		{
			...
			strcpy(addCTHD.MAVT, temp.c_str());
			...
			cout<< "con cho";
			InsertLastCTHD(addHoaDon.PTR_CTHD, addCTHD);
			
			i++;
			temp = "";
		}
		
		InsertLastHoaDon(addNhanVien.PTR_HD, addHoaDon);
